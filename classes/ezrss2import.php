...
/**
 * eZPersistentObject definition for ezrss_import table
 * @copyright Copyright (C) 1999-2010 eZ Systems AS. All rights reserved.
 * @licence http://ez.no/licences/gnu_gpl GNU GPLv2
 * @author Philippe VINCENT-ROYOL
 */

class eZRSS2Import extends eZPersistentObject
{
    ...
    static function definition()
    {
        return array( "fields" => array( "id" => array( 'name' => 'ID',
                                         ...
                                         'import_description' => array( 'name' => 'ImportDescriptionValue',
                                                                        ...
                                                                        'default' => '',
                                                                        'required' => true ),
										'language' => array( 'name' => 'ImportLanguage',
        													'datatype' => 'string',
        													'default' => '',
        													'required' => true )
        														),
    ...
    */
    static function create( $userID = false, $language )
    {
        if ( $userID === false )
        {
    ...
    */
    ...
    /*!
     ...
     \param RSS Import ID
    */
    static function fetch( $id, $asObject = true, $status = eZRSS2Import::STATUS_VALID )
    {
        return eZPersistentObject::fetchObject( eZRSS2Import::definition(),
                                                null,
                                                array( "id" => $id,
                                                       'status' => $status ),
                                                $asObject );
    ...
    */
    static function fetchList( $asObject = true, $status = eZRSS2Import::STATUS_VALID )
    {
        $cond = null;
        if ( $status !== false )
        {
        ...
        return eZPersistentObject::fetchObjectList( eZRSS2Import::definition(),
                                                    null, $cond, null, null,
                                                    $asObject );
    }
    ...
    */
    ...
    {
        $fieldDefinition = eZRSS2Import::rssFieldDefinition();

        $ini = eZINI::instance();
        foreach( $ini->variable( 'RSSSettings', 'ActiveExtensions' ) as $activeExtension )
        {
            if ( file_exists( eZExtension::baseDirectory() . '/' . $activeExtension . '/rss2/' . $activeExtension . 'rssimport.php' ) )
            {
                include_once( eZExtension::baseDirectory() . '/' . $activeExtension . '/rss2/' . $activeExtension . 'rssimport.php' );
                $fieldDefinition = eZRSS2Import::arrayMergeRecursive( $fieldDefinition, call_user_func( array(  $activeExtension . 'rssimport', 'rssFieldDefinition' ), array() ) );
            }
        }

        $returnArray = array();
        eZRSS2Import::recursiveFieldMap( $fieldDefinition, '', '', $returnArray, 0 );
    ...
    */
    static function recursiveFieldMap( $definitionArray, $globalKey, $value, &$returnArray, $count )
    {
        foreach( $definitionArray as $key => $definition )
        {
            ...
            {
                eZRSS2Import::recursiveFieldMap( $definition,
    ...
    */
    ...
    static function arrayMergeRecursive( $arr1, $arr2 )
    {
        ...
        {
            $arr1[$key] = eZRSS2Import::arrayMergeRecursive( @$arr1[$key], $value);
        }
