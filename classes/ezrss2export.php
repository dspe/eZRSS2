...
/**
 * eZPersistentObject definition for ezrss_export table
 * @copyright Copyright (C) 1999-2010 eZ Systems AS. All rights reserved.
 * @licence http://ez.no/licences/gnu_gpl GNU GPLv2
 * @author Philippe VINCENT-ROYOL
 */

class eZRSS2Export extends eZPersistentObject
{
    ...
    */
    function store( $storeAsValid = false )
    {
        ...
        if ( $storeAsValid )
        {
            ...
            $this->setAttribute( 'status', eZRSS2Export::STATUS_VALID );
        }
    ...
    */
    ...
    /*!
     ...
     \param RSS Export ID
    */
    static function fetch( $id, $asObject = true, $status = eZRSS2Export::STATUS_VALID )
    {
        return eZPersistentObject::fetchObject( eZRSS2Export::definition(),
                                                null,
                                                array( "id" => $id,
                                                       'status' => $status ),
                                                $asObject );
    ...
    */
    ...
    {
        return eZPersistentObject::fetchObjectList( eZRSS2Export::definition(),
    ...
    */
    function fetchItems( $id = false, $status = eZRSS2Export::STATUS_VALID )
    {
        if ( $id === false )
        {
            if ( isset( $this ) )
        ...
        if ( $id !== null )
            $itemList = eZRSS2ExportItem::fetchFilteredList( array( 'rssexport_id' => $id, 'status' => $status ) );
        else
            $itemList = null;
        return $itemList;
    }
    ...
    */
    function fetchRSS2_0()
    {
        ...
        $rssSources = eZRSS2ExportItem::fetchFilteredList( $cond );

 
        $nodeArray = eZRSS2ExportItem::fetchNodeList( $rssSources, $this->getObjectListFilter(), $this->Language );
		
		
        if ( is_array( $nodeArray ) && count( $nodeArray ) )
        {
            $attributeMappings = eZRSS2ExportItem::getAttributeMappings( $rssSources );
    ...
    */
    function fetchRSS1_0()
    {
        ...
        $nodeArray = eZRSS2ExportItem::fetchNodeList( $rssSources, $this->getObjectListFilter() );
