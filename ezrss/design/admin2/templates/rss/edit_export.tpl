<form action={"rss2/edit_export"|ezurl} method="post" name="RSSExport">
...
{*                <li>{'If RSS Export is Active then a valid Access URL is required.'|i18n( 'design/admin/rss/edit_export' )}</li>*}
	...
	<div class="block">	
	    <label class="inline" for="rssLanguage">{'Language'|i18n( 'design/admin/rss/edit_export' )}:</label>
	    <select name="Language" id="rssLanguage">
	    	{foreach $languageAvailable as $lg}
	    		<option value="{$lg.locale}" {section show=$rss_export.language|eq( $lg.locale )}selected="selected"{/section}>{$lg.locale}</option>
	    	{/foreach}
	    </select>
    </div>
