...
//fetch this class
$rssImportArray = eZRSS2Import::fetchActiveList();
...
foreach ( $rssImportArray as $rssImport )
{
    ...
    if ( !$isQuiet )
    {
        $cli->output( 'RSS2Import '.$rssImport->attribute( 'name' ).': Starting.' );
    }
    ...
    if ( $xmlData === false )
    {
        if ( !$isQuiet )
        {
            $cli->output( 'RSS2Import '.$rssImport->attribute( 'name' ).': Failed to open RSS feed file: '.$rssSource );
        }
        continue;
    }
    ...
    if ( !$success )
    {
        if ( !$isQuiet )
        {
            $cli->output( 'RSS2Import '.$rssImport->attribute( 'name' ).': Invalid RSS document.' );
        }
        continue;
    }
    ...
    if ( $version != $importDescription['rss_version'] )
    {
        if ( !$isQuiet )
        {
            $cli->output( 'RSS2Import '.$rssImport->attribute( 'name' ).': Invalid RSS version missmatch. Please reconfigure import.' );
        }
        continue;
    }
...
function rssImport1( $root, $rssImport, $cli )
{
    ...
    if ( !$isQuiet )
    {
        $cli->output( 'RSS2Import '.$rssImport->attribute( 'name' ).': End. '.$addCount.' objects added' );
    }
